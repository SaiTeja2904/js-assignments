...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let dot = (x1 * x2) + (y1 * y2);
    let aa = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
    let bb = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
    return Math.acos(dot / (aa * bb));
}
...
function roundToPowerOfTen(num, pow) {
    var x = Math.pow(10, pow);
    var a = num / x;
    return (pow !== 0) ? Math.round(a) * x : num;
}
