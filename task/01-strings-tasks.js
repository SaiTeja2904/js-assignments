...
function getRectangleString(width, height) {
    ...
    stringTopLine += "─".repeat(width-2);
    stringTopLine = stringTopLine.concat("┐\n");
    ans = stringTopLine;
    var middleLines = "";
    for (let i = 0; i < height - 2; i++) {
        ...
        mid = mid.concat("│\n");
        middleLines = middleLines.concat(mid);
    }
    ans = ans.concat(middleLines);
    var stringBottomLine = "└";
    stringBottomLine += "─".repeat(width-2);
    stringBottomLine = stringBottomLine.concat("┘\n");
    ans = ans.concat(stringBottomLine);
    return ans;
}
...
function encodeToRot13(str) {
    var ans="";
    for(let each of str) {
        let charCode = each.charCodeAt();
        let buffer = 0, flag = false;
        ...
        ans += (flag) 
                ?  String.fromCharCode((((charCode-buffer)+13)%26)+buffer) 
                : each;
    }
    return ans;
}
...
function getCardId(value) {
    ...
    let letter = value.substring(0, value.length - 1);
    let symbol = value.charAt(value.length - 1);
